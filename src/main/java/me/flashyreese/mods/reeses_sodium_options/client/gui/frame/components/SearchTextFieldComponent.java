...
public class SearchTextFieldComponent extends AbstractWidget {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float delta) {
        ...
        if (!this.isFocused()) {
            ...
            this.textRenderer.draw(matrixStack, text, this.dim.x() + 6, this.dim.y() + 6, 0xFFAAAAAA);
        }

        this.drawRect(this.dim.x(), this.dim.y(), this.dim.getLimitX(), this.dim.getLimitY(), this.isFocused() ? 0xE0000000 : 0x90000000);
        ...
        if (!string.isEmpty()) {
            ...
            n = this.textRenderer.drawWithShadow(matrixStack, this.renderTextProvider.apply(string2, this.firstCharacterIndex), n, m, 0xFFFFFFFF);
        }
        ...
        if (!string.isEmpty() && bl && j < string.length()) {
            this.textRenderer.drawWithShadow(matrixStack, this.renderTextProvider.apply(string.substring(j), this.selectionStart), n, m, 0xFFFFFFFF);
        }
        // Cursor
        if (this.isFocused()) {
            DrawableHelper.fill(matrixStack, o, m - 1, o + 1, m + 1 + this.textRenderer.fontHeight, -3092272);
        }
        // Highlighted text
        if (k != j) {
            int p = l + this.textRenderer.getWidth(string.substring(0, k));
            this.drawSelectionHighlight(matrixStack, o, m - 1, p - 1, m + 1 + this.textRenderer.fontHeight);
        }
    ...
    private void drawSelectionHighlight(MatrixStack matrixStack, int x1, int y1, int x2, int y2) {
        ...
        DrawableHelper.fill(matrixStack, x1, y1, x2, y2, -16776961);
    }
