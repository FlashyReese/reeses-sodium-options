...
public final class SearchResultOrdering {
     ...
     */
    public static <T> List<T> order(SearchResultOrder order, List<ResourceLocation> resultIds, List<T> items, Function<T, ResourceLocation> idOf) {
        ...
        return switch (order) {
            case PAGE_DISPLAY -> {
                Set<ResourceLocation> resultIdSet = new HashSet<>(resultIds);
            ...
            case RANKED -> {
                Map<ResourceLocation, T> itemsById = new HashMap<>();
                for (T item : items) {
